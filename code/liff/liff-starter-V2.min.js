var liffId = "1656564267-XlA7y2d2",
...
function initLiff(a) {
    console.log("Going to initialize LIFF to", a), liff.init({
        liffId: a
    }).then(() => {
        "liffToken" == getParameterByName("type") && getLiffToken(), "yes" == getParameterByName("auto") && getParameterByName("type") && sendLiffMessage(), initApp()
    }).catch(a => {
...
function initApp() {
    if (console.log("LIFF initialized!"), !liff.isLoggedIn()) {
        ...
        if (!liff.isInClient() && void 0 !== getParameterByName("type")) {
            ...
            c.href = "#", c.id = "liffLogout", c.className = "btn btn-lg btn-danger btn-block", c.innerHTML = "登出", b.insertBefore(c, b.childNodes[6])
        }
        liff.getProfile().then(a => {
            const b = a.displayName;
            console.info("User name is", b), document.getElementById("greet").innerHTML = "你好, " + b + " \u0C7C 感謝你的使用"
        }).catch(a => {
...
function removeElements(a) {
    for (var b = document.getElementsByClassName(a), c = b.length - 1; 0 <= c; c--) b[c] && b[c].parentElement && b[c].id && "data" == b[c].id && b[c].parentElement.removeChild(b[c])
}
